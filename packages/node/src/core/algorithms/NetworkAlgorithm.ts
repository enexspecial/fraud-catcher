...
export class NetworkAlgorithm {
  ...
  private getCountryFromIP(ipAddress: string): string {
    ...
    return 'XX'; // Unknown/suspicious IP
  }
  ...
  private analyzeIPReputation(profile: IPProfile): number {
    ...
    // Check country risk using CountryService
    if (this.countryService.isSuspiciousCountry(profile.country)) {
      riskScore += 0.7; // Very high risk for suspicious countries
    } else if (this.countryService.isVeryHighRiskCountry(profile.country)) {
      riskScore += 0.6; // High risk for very high risk countries
    } else if (this.countryService.isHighRiskCountry(profile.country)) {
      riskScore += 0.4; // Medium risk for high risk countries
    } else {
      // Use country-specific fraud index for normal countries
      const countryRisk = this.countryService.getCountryRiskScore(profile.country);
